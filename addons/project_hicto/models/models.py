...
class Activity(models.Model):
    ...
    name = fields.Char(string='服务概述', required=True)
    project_id = fields.Many2one('project_hicto.project', string='项目', required=True)
    company_id = fields.Many2one('res.partner', string='公司', related='project_id.company_id', readonly=True)
    service_item = fields.Many2one('product.product', string='服务项', domain="[('type', '=', 'service')]", required=True)
    service_way = fields.Selection([
        ('onsite', '现场'),
        ('remote', '远程'),
        ('prepare', '线下准备')
        ], string="服务形式" , default='remote')
    activity_start = fields.Datetime(string='开始时间')
    expert_id = fields.Many2one('res.users', string='组织者')
...
class Timesheet(models.Model):
    ...
    expert_id = fields.Many2one('res.users', string='参与者')
...
class Project(models.Model):
    ...
    company_id = fields.Many2one('res.partner', string='公司', domain="[('is_company', '=', True), ('id', '!=', 1)]", required=True)
    ...
    def _get_status(self):
        ...
        for record in self:
            if record.date_start and record.months:
